...
const PostManagement = ({ userId }) => {
  ...
  const getPosts = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://backend-server-453975176199.us-central1.run.app/admin/posts?page=${page}&includeDeleted=${includeDeleted}`,
        {
          method: "GET",
          headers: {
  ...
  const handleDeletePost = async (post) => {
    try {
      const response = await fetch(`https://backend-server-453975176199.us-central1.run.app/admin/posts/${post._id}/delete`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const handleDeleteComment = async (postId, commentIndex) => {
    try {
      const response = await fetch(`https://backend-server-453975176199.us-central1.run.app/admin/posts/${postId}/moderate-comment`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
              ...
              >
                {post.picturePath && (
                  <CardMedia
                    component="img"
                    height="140"
                    image={`https://backend-server-453975176199.us-central1.run.app/assets/${post.picturePath}`}
                    alt="Post image"
                    sx={{ objectFit: "cover" }}
                  />
                )}
                <CardContent sx={{ p: 1.5, "&:last-child": { pb: 1.5 } }}>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                    <Avatar
                      src={post.userPicturePath ? `https://backend-server-453975176199.us-central1.run.app/assets/${post.userPicturePath}` : undefined}
                      sx={{ mr: 2 }}
                    >
