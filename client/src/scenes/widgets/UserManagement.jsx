...
const UserManagement = ({ userId }) => {
  ...
  const getUsers = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://mockingbird-server-453975176199.asia-south1.run.app//admin/users?page=${page}&search=${searchTerm}`,
        {
          method: "GET",
          headers: {
  ...
  const handleBanUser = async (user) => {
    try {
      const response = await fetch(`https://mockingbird-server-453975176199.asia-south1.run.app//admin/users/${user._id}/ban`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const handleDeleteUser = async (user) => {
    try {
      const response = await fetch(`https://mockingbird-server-453975176199.asia-south1.run.app//admin/users/${user._id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
