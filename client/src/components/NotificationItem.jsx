...
const NotificationItem = ({ notification, onRead, onDelete }) => {
  ...
  const handleMarkAsRead = async (event) => {
    ...
    try {
      const response = await fetch(
        `https://backend-server-453975176199.us-central1.run.app/notifications/${notification._id}/read`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      const response = await fetch(
        `https://backend-server-453975176199.us-central1.run.app/notifications/${notification._id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
