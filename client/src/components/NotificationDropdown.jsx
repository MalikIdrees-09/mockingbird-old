...
const NotificationDropdown = ({ onClose, onUnreadCountChange }) => {
  ...
  const fetchNotifications = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://backend-server-453975176199.us-central1.run.app/notifications?page=${page}&limit=10`,
        {
          method: "GET",
          headers: {
  ...
  const markAllAsRead = async () => {
    ...
    try {
      ...
      const response = await fetch("https://backend-server-453975176199.us-central1.run.app/notifications/mark-all-read", {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
        },
