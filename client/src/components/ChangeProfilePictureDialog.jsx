...
const ChangeProfilePictureDialog = ({ open, onClose, currentPicture, userId }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://backend-server-453975176199.us-central1.run.app/users/${userId}/profile-picture`, {
        method: "PATCH",
        headers: {
    ...
    >
      ...
      <DialogContent>
        <Box textAlign="center" mb={3}>
          ...
          {!showCropper && (
            <Avatar
              src={croppedPreview || (currentPicture ? `https://backend-server-453975176199.us-central1.run.app/assets/${currentPicture}?v=${profilePictureKey}` : undefined)}
              sx={{
                width: 120,
                height: 120,
                mx: "auto",
