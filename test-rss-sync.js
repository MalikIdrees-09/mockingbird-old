async function testRSSSync() {
  try {
    ...
    const statusResponse = await fetch('http://localhost:5000/api/rss/status');
    ...
    const testResponse = await fetch('http://localhost:5000/api/rss/test');
    ...
    const syncResponse = await fetch('http://localhost:5000/api/rss/sync', {
      method: 'POST',
      headers: {
